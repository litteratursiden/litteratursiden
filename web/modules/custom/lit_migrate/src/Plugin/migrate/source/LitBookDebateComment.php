 ...
 * )
 */
class LitBookDebateComment extends DrupalSqlBase {
  ...
  public function query() {
    ...
    $query->fields('n', ['type']);
    $query->innerJoin('content_field_reference_book', 'f', 'n.nid = f.nid');
    $query->fields('f', ['field_reference_book_nid']);
    $query->condition('n.type', 'group_readbook_posts');
  ...
  public function prepareRow(Row $row) {
    $row->setSourceProperty('field_name', 'field_book_debates');
    $row->setSourceProperty('comment_type', 'debate');
